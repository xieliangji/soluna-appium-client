...
| 截图像素空间 | `PixelRect` | 解码后图像的像素；原点为图像左上角，X 向右、Y 向下 | `ViewportRect`、Screenshot、OCR/CV |

`Rect`/`Point` 和 `PixelRect` 不可互换。尤其是名称都包含 viewport 时，
W3C Actions 的 viewport 与 Appium `mobile: viewportRect` 返回的截图像素
viewport 仍然属于不同空间。
...
- `Rect` 的 `X`、`Y`、`Width`、`Height` 使用 `float64`，保留远端 WebDriver
  返回的连续坐标语义。元素矩形可以有小数坐标，不能据此推断截图像素。
...
`Rect` 的现有行为保持不变：它承载 WebDriver 的连续值，不承担截图像素边界
校验；引入 `PixelRect` 不会改变其 `float64` 形态，也不会把 `PixelRect` 的单位
或边界规则附加到 `Rect`。对 `Rect` 的查找和点击校验仍由现有 Element 行为负责。
...
| `Session.Screenshot`、`Element.Screenshot` | 图像产物 | 返回 Driver 生成的字节，不承诺与 `Rect` 一一对应 |
| `Session.ViewportRect` | 截图像素 | 供调用方解释 Screenshot 图像区域；不改变上面任何 API |

## 3. 截图像素空间与 `PixelRect`

### 3.1 类型语义

`PixelRect` 是一个整数、轴对齐、半开的图像区域。其公共字段沿用 `Rect` 的
`X`/`Y` 命名，但语义不同：

- `X`、`Y` 是图像像素缓冲区中的左、上偏移，不是 WebDriver 点或 CSS 像素；
- `Width`、`Height` 是像素数量，不是右、下坐标；
- 有效区域是 `[X, X+Width) × [Y, Y+Height)`；右边界和下边界本身不属于区域；
- 零值 `PixelRect{}` 不表示一个可用 viewport。远端返回的 viewport 必须有
  正面积，无法确定有效区域时返回错误；
- 类型不携带截图编码、颜色通道、orientation、scale、status bar 或 Context
  信息。需要这些事实时由调用方单独取得并记录。

`ViewportRect` 是 Driver 在一次调用时报告的 `PixelRect` 快照。它描述 Driver
认为属于当前移动设备 viewport 的图像区域，不承诺一定等于下一次 Screenshot
的完整图像边界。上述 Native Context 语义不延伸为 Web Context 的 DOM/CSS
viewport 语义；Web Context 的关系由 `DP-090` 单独定义。Screenshot 的实际
像素宽高始终以解码图像自身为准。
...
|---|---|---|---|
| `XCUITest` | `getViewportRect` | 以 WDA screen info 的 status bar 和 scale、以及当前 WebDriver Window Rect 计算 | Screenshot/device pixels |
| `UiAutomator2` | `mobileViewPortRect` | 以当前 Android window/display 尺寸和 system bars 的 status bar 高度计算 | Driver 报告的 display pixel units |

表中的公式是对设计时上游 Driver 公开实现的行为模型，用来解释返回值的单位和
边界；它们不是 SDK 端重新计算 viewport 的算法。Driver 版本、Context 或设备
状态变化后，以远端实际响应为准，真实组合的结果再登记到
...
这组事实说明为什么不能把 XCUITest 的 `Rect` 直接拿来索引 Screenshot：同一
个 Driver 的 Window/Element 坐标和 ViewportRect 坐标可能相差 scale，并且
status bar 只在像素 viewport 中表现为顶部偏移。
...
因此 Android 的 `PixelRect` 也只是 Driver 报告值的严格承载。即使某一设备上
它恰好与 Screenshot 的尺寸一致，也不构成跨 Android API、导航模式或 Driver
版本的等价承诺。
...
宽高做隐式跨命令比较。若 `right/bottom` 超出截图边界，这是兼容性事实或时序
不一致，必须由调用方在验证流程中记录，不能由 SDK 裁剪或移动矩形。
...
DP-060 只确定验证方法，不宣称任何真实环境已经 `Verified`。对每个要支持的
Appium/Driver/设备组合，应在同一稳定 UI 状态下：

1. 记录 SDK、Appium、Driver、WDA 或 UiAutomator2 Server、设备 OS、真机/模拟器
   和 Host OS 版本；
2. 固定 orientation，并连续读取 `ViewportRect` 与 `Screenshot`，记录读取时间；
3. 解码 Screenshot，取得实际像素宽高；检查 `PixelRect` 是否为非负、正面积，
   并记录 `X+Width <= imageWidth`、`Y+Height <= imageHeight` 是否成立；
4. 在 portrait/landscape、status bar 可见/隐藏以及适用的真机/模拟器条件下
   重复，分别核对 XCUITest 的 scale/status bar 影响和 UiAutomator2 的
   system-bars 影响；
5. 若 viewport 与 Screenshot 不在同一像素平面，保留原始版本和结果作为兼容性
   差异，停止任何基于该结果的隐式换算；
6. 将真实结果写入 `docs/compatibility.md` 后，才能把能力的验证状态改为
   `Verified`。协议测试通过本身不改变该状态。

该流程是诊断和兼容性验证，不是普通命令的运行时门禁，也不要求每次业务调用
同时读取 Screenshot。

## 9. 被拒绝的方案

- **复用 `Rect` 表示像素区域**：同名字段仍会隐藏逻辑单位、scale 和半开整数
  边界，调用方容易把 Screenshot 像素传给 Actions，因此使用独立 `PixelRect`。
...
- **缓存 ViewportRect 或与 Screenshot 自动绑定**：方向、系统栏和页面状态可在
  两次命令之间变化；SDK 不建立跨命令快照或串行器。
- **在本地裁剪 Screenshot 或提供 Viewport Screenshot**：不同 Driver 的图像
  尺寸、方向和像素密度尚未有统一等价承诺，Viewport Screenshot 仍是 `VIS-007`
  的 Deferred 能力。
- **把 ViewportRect 接入 Find/Tap**：会把图像像素与 WebDriver 坐标混用，破坏
  现有 Native 几何和 Actions 契约。
...
- 在根包定义 `PixelRect` 和 `Session.ViewportRect`，不增加平行平台对象；
