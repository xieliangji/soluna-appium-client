...
| VIS-006 | `Session.ViewportRect` / `PixelRect` | Session method | Accepted | Appium mobile viewport rect | XCUITest / UiAutomator2；精确 AutomationName 门禁；当前坐标契约以 Native Context 为主；Driver 像素几何不绑定具体 Screenshot，具体 Screenshot 像素关系待 compatibility 验证；不参与 Element Find/Tap，也不执行隐式转换 | None | DP-060 已完成 `docs/coordinate-system.md`；DP-061 实现并补协议测试 |
