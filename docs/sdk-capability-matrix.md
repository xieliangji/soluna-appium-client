...
| DISC-001 | 读取 Command Catalog | Session method | Accepted | Appium Runtime Discovery | Appium 3 | None | DP-040 已固定 `rest`/`bidi`、必需 `base`/`driver`、可选 `plugins[name]` 层级，HTTP/BiDi execution identity 及 `params[].required`；DP-041 实现 `Session.Commands` |
| DISC-002 | 读取 Extension Catalog | Session method | Accepted | Appium Runtime Discovery | Appium 3 | None | DP-040 已固定 `rest.driver`/`rest.plugins[name]`、必需 `driver` child、Execute Method execution identity 及 `params[].required`；DP-041 实现 `Session.Extensions` |
| DISC-003 | Catalog 本地 `Supports` helper | Session method | Accepted | 纯本地快照查询 | 不能视为执行成功保证 | None | DP-040 已固定按 HTTP、BiDi、Execute Method 分开的精确 execution identity 匹配规则，Source 仅作 provenance；DP-041 实现 helper |
...
| ELM-006 | `Element.Screenshot` | Element method | Accepted | W3C Element Screenshot | 不承诺自动滚动或与本地裁剪等价 | None | 与 Screenshot 资源重构一起实现 |
| ELM-007 | `Element.ScreenshotTo(io.Writer)` | Element method | Accepted | 流式 Base64 解码 | 使用 Screenshot 专用上限 | None | 复用 Session Screenshot 解码链 |
...
| VIS-002 | `Session.Screenshot` | Session method | Implemented | W3C Screenshot | 当前仍使用普通响应上限 | Protocol | `session.go`, `session_inspection_test.go` |
| VIS-003 | `Session.ScreenshotTo(io.Writer)` | Session method | Accepted | 流式 Base64 解码 | 增加 `MaxScreenshotResponseBytes` | None | 从 `session.go` 拆入截图领域 |
| VIS-004 | Screenshot 专用资源上限 | Client option / callback | Accepted | Client Limits | Session/Element Screenshot 共用 | None | 扩展 `Limits` |
...
| INF-003 | 响应和远端错误大小限制 | Client option / callback | Implemented | `Limits` | Screenshot/Logs/BiDi 仍待扩展 | Protocol | `limits.go`, transport/codec tests |
| INF-004 | Base64 流式解码 | Internal / test infrastructure | Implemented | `internal/codec` | 已用于 Recording | Unit/Protocol | `internal/codec`, `recording.go` |
