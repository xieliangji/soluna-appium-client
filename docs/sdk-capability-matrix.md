...
| ID | 能力 / 目标 API | 公共入口 | 状态 | 机制 | Host/版本约束 | 验证 | 证据或下一步 |
|---|---|---|---|---|---|---|---|
| ELM-001 | `Session.Find` / `FindElements` | Session method | Implemented | W3C plural element lookup + Context-specific geometry | Native 使用 Window Rect；Web 以滚动快照将文档相对 Rect 平移到 CSS layout viewport；真实 Safari/Hybrid 组合待验证 | Protocol | `context.go`, `element.go`, `element_find_test.go`, `element_context_test.go`, `docs/design.md`, `docs/coordinate-system.md` |
| ELM-002 | Element 作用域 `Find` / `FindElements` | Element method | Implemented | W3C plural find from element + Context-specific geometry | Native 使用 Window Rect；Web 以滚动快照将文档相对 Rect 平移到 CSS layout viewport；真实 Safari/Hybrid 组合待验证 | Protocol | `context.go`, `element.go`, `element_find_scope_test.go`, `element_context_test.go`, `docs/design.md`, `docs/coordinate-system.md` |
| ELM-003 | `Element.Rect` / `Text` / `Attribute` | Element method | Implemented | W3C Element commands | Web 中直接 Rect 保留 WebDriver 文档相对 CSS pixel 语义；仅 Context-sensitive Find/Tap 按 viewport 平移；Attribute 允许远端 `null` | Protocol | `context.go`, `element.go`, `element_test.go`, `element_context_test.go`, `docs/design.md`, `docs/coordinate-system.md` |
...
| ELM-005 | `Element.Tap` / `TapInWindowIntersection` | Element method | Implemented | Context-specific Rect intersection + W3C Actions | Native 使用 Window Rect；Web 使用滚动平移后的 CSS layout viewport；每次点击重新读取 Context 与几何状态；真实组合待验证 | Protocol | `context.go`, `element.go`, `element_tap_test.go`, `element_context_test.go`, `docs/design.md`, `docs/coordinate-system.md` |
...
| KBD-001 | `Session.KeyboardShown` | Session method | Implemented | Appium common command `is_keyboard_shown`；每次返回不缓存的 Driver 状态快照；Operation/Observer identity 固定为 `keyboard_shown`；成功值严格为 JSON boolean | XCUITest 通过键盘元素探测；UiAutomator2 读取输入法状态；false 不等于永久或像素级事实 | Protocol | `keyboard.go`, `keyboard_test.go`, `docs/design.md`, `docs/command-semantics.md`, `docs/error-model.md` |
| KBD-002 | `Session.DismissKeyboard`（`(bool, error)`） | Session method | Implemented | Appium common command `hide_keyboard`；固定 `{}` 请求；严格校验并返回原始 Driver boolean；Operation/Observer identity 固定为 `dismiss_keyboard`；不把结果当最终状态 | 两 Driver 的关闭策略和 true/false 语义不同；内部 Done/ESC/BACK 可能改变应用状态；不提供 strategy/特殊键、IME 管理或 fallback；当前 Appium 3.6.0/XCUITest Driver 12.1.0 route 仍存在但 deprecated，移除后只返回统一远端错误 | Protocol | `keyboard.go`, `keyboard_test.go`, `docs/design.md`, `docs/command-semantics.md`, `docs/error-model.md`；真实版本/设备/Host 组合仍待兼容性记录 |

> KBD-002 当前仍只有 Appium 3.6.0 + XCUITest Driver 12.1.0 的上游源码观察：
> common `hide_keyboard` route 仍注册但已 deprecated。这不是已支持的版本范围，
> 也不是 `Verified` 证据；后续兼容性验证必须在 `docs/compatibility.md` 逐项记录实际
...
| NAV-003 | `Session.DeepLink` | Session method | Implemented | 固定 `mobile: deepLink` Execute Method；按远端确认的精确 AutomationName 映射 XCUITest `bundleId` / UiAutomator2 `package`；空 appID 省略目标；成功 value 严格为 `null`，固定 `deep_link` identity | XCUITest 4.17+ 且 iOS 16.4+ / Xcode 14.3+；UiAutomator2 的 package 自 3.9.3 起可省略；未知 Driver 本地拒绝，不补全目标、不规范化、不重试、不做页面断言或 fallback | Protocol | `navigation.go`, `deep_link_test.go`, `docs/command-semantics.md`, `docs/error-model.md`；协议依据 XCUITest 12.1.0 / UiAutomator2 8.2.0 上游源码，真实版本/设备/Host 组合仍待兼容性记录 |
...
| EXEC-001 | `ExecuteScript` / `ExecuteScriptWithOperation` / `ExecuteScriptWithOperationAndDecode` | Session method | Implemented | W3C Execute Script；高级调用方可提供格式有界的本地 operation identity 与 value decoder | `mobile:` 扩展逃生口；不开放任意 Method/Route；decoder 错误进入统一执行链 | Protocol | `execute.go`, `execute_test.go`；AD-027、AD-028 |
...
| BIDI-001 | 通用 WebDriver BiDi 连接与命令关联 | Session stream (`Session.Subscribe`) | Accepted | Session 所有的单 WebSocket 连接；内部固定命令与 ID 关联 | 远端确认 webSocketUrl；首版按 Appium 3.0.0 原生 BiDi 协议，不承诺浏览器代理模式；不恢复失效连接 | None | DP-170 / `docs/design.md` §10.4、AD-036；已确定 Endpoint、状态与内部 coder/websocket v1.8.14 依赖选择，DP-171 才实现 |
| BIDI-002 | 有界订阅、取消、背压和关闭；`BiDiSubscription`、`EventStream.Next/Close/Done/Err`、`BiDiEvent`、`BiDiLimits` | Session stream / Client option | Accepted | Events/Contexts 对称订阅和取消；事件名跨流互斥、单消费者；单消息/队列/累计预算及可观察终止 | nil Contexts 使用 Appium 默认 `[""]`，显式列表精确匹配；不自动发现 Context、重连/重订阅或清理平台监控状态 | None | DP-170 / `docs/design.md` §10.4；contexts P1 修订待复审；请求与错误见命令语义/错误模型；API、Limits 与流错误码均待 DP-171 实现 |
...
| XCUI-004 | `IOSAcceptAlertWithLabel` / `IOSDismissAlertWithLabel` | Platform function | Implemented | `mobile: alert` 的 `accept` / `dismiss` 与显式 `buttonLabel`；统一 Execute Script 链 | iOS/XCUITest；非空且有效 UTF-8 label；版本/Host 依据见 `docs/compatibility.md` | Protocol | `xcuitest/alerts.go`, `xcuitest/alerts_test.go`, `docs/command-semantics.md`；只提供 label 增量 |
| XCUI-005 | `IOSGetSimulatedLocation` / `IOSSetSimulatedLocation` / `IOSResetSimulatedLocation` | Platform function | Implemented | WDA simulated location；固定 Execute Method 与严格位置 decoder | XCUITest Driver 4.18+、Xcode 14.3+、iOS 16.4+；iOS 17 主要 macOS；真机/Host 组合见 `docs/compatibility.md` | Protocol | `xcuitest/location.go`, `xcuitest/location_test.go`, `docs/command-semantics.md` |
...
| UIA-002 | Android 物理键 | Platform function | Deferred | `mobile: pressKey`；keycode、metastate、长按和输入源 | Android API level、设备厂商与 Driver/Server 版本影响结果；Appium HTTP 路径不绑定 SDK Host | None | DP-161 评审：W3C Touch Actions 无可靠替代，但仓库没有明确业务场景或设备矩阵；仅在真实调用场景确认后纳入 |
| UIA-003 | Android 专有滚动/手势 | Platform function | Deferred | `mobile: scrollGesture` / `mobile: flingGesture`；容器边界与 `canScrollMore` 语义 | Appium HTTP 路径可跨 Host；Android UI、UiAutomator2 Server 与设备版本影响行为 | None | DP-161 评审：`Session.Swipe` 可覆盖坐标级手势，但仓库没有容器滚动/惯性场景；不批量复制 Gesture API |
| UIA-004 | Android 系统面板/通知 | Platform function | Deferred | `mobile: openNotifications` / `mobile: statusBar` / `mobile: getNotifications` | Android API、OEM 系统面板、Appium Settings 通知权限和设备授权影响结果；不绑定 SDK Host 但设备条件差异大 | None | DP-161 评审：通用 Actions 无可靠系统面板替代，但仓库没有通知业务场景或权限验证；需明确高价值场景后再纳入 |
...
| INF-006 | BiDi Contract Test 工具 | Internal / test infrastructure | Accepted | Fake WebSocket/BiDi Server，与 HTTP Session fixture 协作 | Test-only；合成数据与确定性同步 | None | DP-170 / `docs/design.md` §10.4.8 已定义 context 精确转发/对称取消、关联、限额、关闭及 race 验收；DP-171 实现，当前没有 Fake Server 或 BiDi 协议测试 |
