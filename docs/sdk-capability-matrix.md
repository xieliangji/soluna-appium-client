...
| CFG-002 | 读取当前 Timeouts | Session method | Implemented | Appium 3 Get Timeouts (`command` / `implicit`) | 字段必须存在；值允许 `null`，非空值严格校验整数毫秒、非负和 `time.Duration` 溢出 | Protocol | `timeouts.go`, `timeouts_test.go`, `docs/command-semantics.md` |
