...
| ALERT-001 | Alert 文本 | Session method | Accepted | W3C Get Alert Text | 增加 `CodeAlertNotFound` 评审 | None | 新增 `alerts.go` |
| ALERT-002 | Accept / Dismiss Alert | Session method | Accepted | W3C Alert commands | 不增加 `HasAlert` TOCTOU API | None | 新增 `alerts.go` |
| ALERT-003 | Set Alert Text | Session method | Accepted | W3C Set Alert Text | 仅含输入框 Alert 有效 | None | 新增 `alerts.go` |
