...
| BIDI-002 | 有界订阅、取消、背压和关闭；`EventStream.Next/Close/Done/Err`、`BiDiEvent`、`BiDiLimits` | Session stream / Client option | Accepted | 精确事件名互斥多订阅、单消费者；单消息/队列/累计预算及可观察终止 | Appium 默认事件作用域；不自动重连/重订阅，不清理平台监控业务状态 | None | DP-170 / `docs/design.md` §10.4；请求与错误见命令语义/错误模型；API、Limits 与流错误码均待 DP-171 实现 |
...
| INF-006 | BiDi Contract Test 工具 | Internal / test infrastructure | Accepted | Fake WebSocket/BiDi Server，与 HTTP Session fixture 协作 | Test-only；合成数据与确定性同步 | None | DP-170 / `docs/design.md` §10.4.8 已定义关联、取消、限额、关闭及 race 验收；DP-171 实现，当前没有 Fake Server 或 BiDi 协议测试 |
